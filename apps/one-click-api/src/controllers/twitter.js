...
export const twitterPost = async (req, res) => {
    try {
        ...
        const post = await client.v1.tweet(req.body.message, { media_ids: mediaId });
        console.log(post)
        res.send(post);
    }
    catch (e) {
        ...
        try {
            ...
            if (imageObjList && imageObjList.length) {
                for (let img of imageObjList) {
                    ...
                    const mediaId = await client.v1.uploadMedia(buffer, { mimeType: img.fileType });
...
export const getTwitterPost = async (req, res, next) => {
    try {
        ...
        try {
            const twitter = await axios.get(`${environment.twitter_url}/users/${req.body.id}/tweets?expansions=author_id,attachments.media_keys&media.fields=url&tweet.fields=created_at,entities&user.fields=url`, { headers: header })
...
function getTwitterToekFromV1ToV2() {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const twitter = await axios.post(`https://api.twitter.com/oauth2/token?grant_type=client_credentials`, { "grant_type": 'client_credentials' }, {
                headers: {
